...
describe("SimpleVoting", function () {
    ...
    describe("Creating a ballot", function () {
        ...
        it("should return the number of ballots", async () => {
            const { contract } = await loadFixture(deploy)
            const startTime = await time.latest() + 60
            const duration = 300
            const question = "Who is the greatest rapper of all time?"
            ...
            await contract.createBallot(question, options, startTime, duration)
            expect(await contract.getNumberofBallots()).to.be.equal(1);
        })
        ...
        describe("Tallying votes", async () => {
            ...
            it("should return the results for every option", async () => {
                await time.increase(duration + 1)
                expect(await contract.results(0)).to.deep.eq([
                    2n, 1n, 1n, 0n
                ])
            })
            it("should return the winner for a ballot", async () => {
                await time.increase(duration + 1);
                expect(await contract.winners(0)).to.deep.eq([true, false, false, false])
            })
            it("should return multiple winners for a tied ballot", async () => {
                const signers = await ethers.getSigners()
